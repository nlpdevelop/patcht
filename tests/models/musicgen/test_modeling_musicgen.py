...
class MusicgenTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    # Not to test with `MusicgenForConditionalGeneration` as the model tester doesn't prepare `input_values` and
    # `padding_mask` (and `torchscript` hates `None`).
    def _create_and_check_torchscript(self, config, inputs_dict):
        ...
        for model_class in self.all_model_classes:
            if model_class.__name__ == "MusicgenForConditionalGeneration":
                continue
